	...
	def checkError(self, type, poly=""):
		...
		if type == 1:
			for i in range(len(self.codeword)):
				...
				for j in range(len(self.codeword[i])):
					if self.codeword[i][j]=='1':
						countOnes += 1
				if countOnes%2==0:
					print("the Parity of recived packet is even.", end=' ')
					print("NO ERROR DETECTED")
				else:
					print("the Parity of recived packet is odd.", end=' ')
			...
			while i<self.dataSize:
				...
				i+=1
			if error:
				print("Parity of recived packet did not match.",end=' ')
				print("ERROR DETECTED")
			else:
				print("Parity of recived packet matched.",end=' ')
				print("NO ERROR DETECTED")
