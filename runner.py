...
def insertSpecificError(frames, zeropos, onepos):
...
def case1(size, filename):
	puterror = True
	print("--------------------------- CASE 1 -------------------------")
	print("here Error will be  detected by all four schemes.")
	...
	if puterror:
		s.codeword = insertRandomError(s.codeword)
	r = Receiver(s)
	r.checkError(type)
...
def case2(size, filename):
	...
	zeropos = []
	onepos = [[5]]
	s.codeword = insertSpecificError(s.codeword,zeropos,onepos)
	r = Receiver(s)
	r.checkError(type)
	r.displayFrames(type)
	print("-------------- Cyclic Redundancy Check ---------------------")
...
if __name__ == "__main__":
	...
	if case == 1:
		...
		noOfDataWord = int(input("Enter the number of data words : "))
		generateRandomInput(size*noOfDataWord, sys.argv[1])
